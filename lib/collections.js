...
module.exports = function(config) {
  ...
  */
  ...
  // Helper to get visible sponsors
  const getVisibleSponsors = (collectionApi) => {
    return collectionApi.getFilteredByGlob("src/_content/sponsors/*.md")
      .filter(item => !item.data.hidden);
  };

  config.addCollection("sponsors", function(collectionApi) {
    return getVisibleSponsors(collectionApi);
  });

  config.addCollection("sponsorsByLevel", function(collectionApi) {
    const visibleSponsors = getVisibleSponsors(collectionApi);
